...
const srvName = "product-catalog-web-node-ipc-server";
...
client.connectTo(srvName, () => {
    client.of[srvName].on("connect", () => {
        client.of[srvName].emit("product-catalog-web-node-instance-started");
...
function startServer() {
    ...
    server.serve(() => server.server.on("product-catalog-web-node-instance-started", () => {
        process.exit(1);
    }));
    server.server.start();
